...
const Cart = {
    ...
    init: async function() {
        ...
        if (!window.App.currentUser) {
            console.log('No user, showing empty cart with login prompt');
            this.renderEmptyCartWithLogin();
            return;
        }
    ...
    renderEmptyCartWithLogin: function() {
        const container = document.getElementById('cart-items');
        if (container) {
            container.innerHTML = `
                <div style="text-align: center; padding: 60px;">
                    <h3>🛒 Корзина пуста</h3>
                    <p>Войдите в аккаунт, чтобы увидеть добавленные товары</p>
                    <button class="btn btn--primary" id="login-prompt-btn" style="margin-top: 20px;">Войти</button>
                </div>
            `;
        ...
        container.innerHTML = this.items.map(item => `
            <div class="cart-item" data-id="${item.productId}">
                <div class="cart-item__image">
                    <img src="${item.image || 'assets/img/product-cart-img1.png'}" alt="${item.name}" onerror="this.src='https://via.placeholder.com/80'">
                </div>
    ...
    initEvents: function() {
        ...
        if (checkoutBtn) {
            checkoutBtn.addEventListener('click', async () => {
                const success = await window.App.createOrder();
                if (success) {
                    await this.loadCart();
                    this.renderCart();
                    this.initEvents();
