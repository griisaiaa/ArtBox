...
const ProductInfo = {
    ...
    init: async function() {
        console.log('ProductInfo.init() started');
    ...
    loadProduct: async function() {
        try {
            const productRef = doc(window.App.db, "products", this.productId);
        ...
        container.innerHTML = `
            <div class="product-gallery">
                <div class="gallery-main">
                    <img src="${this.product.image || './assets/img/tovar-info-main-img.png'}" alt="${this.product.name}" class="gallery-main__img">
                </div>
            </div>
            <div class="product-info">
                ...
                <div class="product-actions">
                    <button class="btn btn--cart btn--full" id="add-to-cart-btn">Добавить в корзину</button>
    ...
    checkFavorite: async function() {
        if (window.App.currentUser) {
            const isFav = window.App.isFavorite(this.productId);
            const btn = document.getElementById('favorite-btn');
            if (btn && isFav) {
                btn.classList.add('active');
                btn.querySelector('img').src = './assets/img/tovar-info-favourite-icon-active.png';
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('ProductInfo: DOM loaded');
    setTimeout(() => {
        ProductInfo.init();
    }, 200);
});